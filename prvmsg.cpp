...
void Server::broadcastToChannel(const std::string &channel_name, const std::string &host, const std::string &sender, const std::string &message)
{
    ...
    if (channel_it != _channels.end())
    {
        ...
        if (clients_in_channel.find(sender) == clients_in_channel.end())
        {
            ...
            sendReply(client, ERR_CANNOTSENDTOCHAN(host, sender, channel_name));
            return;
        }
    ...
    {
        sendReply(_clients[getClientByNickname(sender)].getClientFd(), ERR_NOSUCHCHANNEL(host, sender, channel_name));
    }
}

void Server::sendToClient(const std::string &target_nick, const std::string &host, const std::string &sender_nick, const std::string &message)
{
    ...
    {
        sendReply(_clients[getClientByNickname(sender_nick)].getClientFd(), ERR_NOSUCHNICK(host, sender_nick, target_nick));
    }
}
...
void Server::PrivMsgCommand(int client_fd, std::vector<std::string> command, std::string &buffer)
{
    ...
    std::string sender_host = _clients[client_fd].getHostName();
    ...
    for (size_t i = 0; i < target_list.size(); ++i)
    {
        ...
        if (target[0] == '#')
        {
            broadcastToChannel(target, sender_host, sender_nick, message);
        }
        else
        {
            sendToClient(target, sender_host, sender_nick, message);
        }
    }
}